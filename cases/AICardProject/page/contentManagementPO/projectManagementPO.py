...
class projectManagement(BasePage):
    control = {
        ...
        "关闭项目接待页面":(By.XPATH,'//div[@class="ant-modal-content"]/button[@class="ant-modal-close"]'),
        #操作-复制页面路径列表
        "操作-复制页面路径":(By.XPATH,'//tr/td[6]/button[@class="ant-btn ant-btn-link ant-btn-sm" ][3][ not(@disabled)]'),
        ...
        if self.is_exist_element(self.control["发布状态-列表"]):
            ...
            for l in list:
                ...
                if l.text != publishStatus :
                    ...
                    if self.is_exist_element(self.control['提示弹窗-取消']):
                        sleep(2)
                        self.find_element(self.control['提示弹窗-取消']).click()
    ...
    def select_city(self):
        ...
        lists = self.find_elements_list(self.control['所属城市-第二个列表'])
        sleep(2)
        self.find_elements(self.control['所属城市-第二个列表'],len(lists)-1).click()
